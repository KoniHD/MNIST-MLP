...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
        model = Module{seed};
    MPI_Bcast(model.W1.data(), INPUT_DIM * HIDDEN_DIM, MPI_FLOAT, 0, MPI_COMM_WORLD);
    MPI_Bcast(model.b1.data(), HIDDEN_DIM, MPI_FLOAT, 0, MPI_COMM_WORLD);
    MPI_Bcast(model.W2.data(), HIDDEN_DIM * OUTPUT_DIM, MPI_FLOAT, 0, MPI_COMM_WORLD);
    MPI_Bcast(model.b2.data(), OUTPUT_DIM, MPI_FLOAT, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        Utility::write_prediction_montage("predictions.ppm", test_images, test_preds, test_labels_u8, 16);
